...
public class SoapHelper {
  ...
  /* public Set<IssuesData> getIssues() throws MalformedURLException, ServiceException, RemoteException { //TODO
      MantisConnectPortType mc = getMantisConnect();
      ProjectData[] projects = mc.mc_projects_get_user_accessible(app.getProperty("web.adminLogin"), app.getProperty("web.adminPassword"));
      IssueData[] issues = Arrays.stream(projects).map((p) -> p.)
      return Arrays.asList(projects).stream().map((p) -> new Project().withId(p.getId().intValue()).withName(p.getName())).collect(Collectors.toSet());
   }*/

   public MantisConnectPortType getMantisConnect() throws ServiceException, MalformedURLException {
      return new MantisConnectLocator()
                 .getMantisConnectPort(new URL(app.getProperty("mantis.soaplink")));
   }

   public IssuesData addIssue(IssuesData issue) throws MalformedURLException, ServiceException, RemoteException {
      MantisConnectPortType mc = getMantisConnect();
